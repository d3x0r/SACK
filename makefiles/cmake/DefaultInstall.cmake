  ...
  if( NOT SOURCES_ROOT )
	...
	if( __ANDROID__ )
		...
		if( ${option1} STREQUAL WIN32 )
	     		set( BUILT_A_ANDROID_CONSOLE_APP 0 )
			if( portable )
			...
			if( portable )
				...
				add_library( ${targetname}.code SHARED ${option1} ${ARGN} ${SOURCES_ROOT}/src/deadstart/deadstart_core.c )
                                if( BUILD_ANDROID_CONSOLE_APPS )
					set( BUILT_A_ANDROID_CONSOLE_APP 1 )
                                	add_executable( ${targetname}.exe ${option1} ${ARGN} )
                                endif( BUILD_ANDROID_CONSOLE_APPS )
			else( portable )
		...
		set_target_properties( ${targetname} PROPERTIES COMPILE_DEFINITIONS "${ExtraDefinitions};ANDROID_SHELL;TARGETNAME=\"${targetname}\";TARGET_LABEL=${TARGET_LABEL}" )
                if( BUILT_A_ANDROID_CONSOLE_APP )
			my_target_link_libraries( ${targetname}.exe ${SACK_LIBRARIES} ${SACK_PLATFORM_LIBRARIES} )
			set_target_properties( ${targetname}.exe PROPERTIES COMPILE_DEFINITIONS "${ExtraDefinitions};ANDROID_CONSOLE_UTIL;TARGETNAME=\"${targetname}.exe\";TARGET_LABEL=${TARGET_LABEL}_exe" )
			install_default_dest( ${targetname}.exe )
                endif( BUILT_A_ANDROID_CONSOLE_APP )
