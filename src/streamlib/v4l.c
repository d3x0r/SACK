...
//  														, width, height ) );
      ...
      UpdateDisplay( pRender );
		AddCaptureCallback( g.pDev[0], DisplayAFrame, (PTRSZVAL)pRender );
#ifdef __LINUX__
		...
		{
			AddCaptureCallback( g.pDev[1], DisplayASmallFrame, (PTRSZVAL)pRender );
		}
#endif
		//if(0)
#ifdef __LINUX__
...
//  			AddCaptureCallback( g.pDev[0], RenderNetworkFrame, OpenNetworkRender( NULL ) );
   ...
   SetAllocateLogging( TRUE );
		{
			char whatever[2];
			do{lprintf( WIDE("Entering wait...") ); fgets( whatever, 2, stdin ); } while( whatever[0] != 'q' );
		}
