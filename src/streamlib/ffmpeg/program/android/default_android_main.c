...
static void (*VideoPlayer_PlayMedia)(char* );
static char *myname;
static char *mypath;
...
extern "C"
{
	...
	JNIEXPORT void JNICALL
		Java_org_d3x0r_sack_core_NativeStaticLib_playMedia(JNIEnv * env, jobject obj, jstring media )
	{
      int fd = -1;
		//LOGI( "SETTING PACKAGE: %s", package );
		// there is a release function for this... so this will be always valid.
		// restart should change this?
		const char *file = (*env)->GetStringUTFChars( env, media, NULL );
