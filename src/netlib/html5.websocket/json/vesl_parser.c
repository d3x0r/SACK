				...
				if( state->parse_context == CONTEXT_OBJECT_FIELD ) {
					...
					if( c < 0xFF ) {
						if( c == ':' ) {
							...
							if( state->val.string ) {
								if( state->val.name ) {
									vtprintf( state->pvtError, WIDE( "fault while parsing object field name  a second colon?; \\u00%02X unexpected at %" ) _size_f WIDE( "  %" ) _size_f WIDE( ":%" ) _size_f, c, state->n, state->line, state->col );	// fault
									state->status = FALSE;
									break;
								}
								state->word = WORD_POS_RESET;
