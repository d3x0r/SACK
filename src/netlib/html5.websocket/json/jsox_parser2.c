...
void process_jsox_state( struct jsox_parse_state *state
                       , PJSOX_PARSE_BUFFER input
) {
	TEXTRUNE c;
	PLINKQUEUE plqInputs = NULL;
	int string_status;
	int retval = 0;
	if( !state->status )
		return -1;
	...
	while( state->status && ( input = (PJSOX_PARSE_BUFFER)DequeLinkNL( state->inBuffers ) ) ) {
		...
		while( state->status && ((input->pos-input->buf) < input->size) && ( (c = GetUtfChar( &input->pos ))!= BADUTF8) )
		{
			...
			if( escape ) {  // some escapes in strings are multichar \u{1029a}; 
			                //but we just need to ingore \<quote>, one character
				escape = 0;
				continue;
			}
			if( quote ) {
				   ...
				   || state->parse_context == JSOX_CONTEXT_CLASS_FIELD
					...
					default:
						...
						if( isNonIdentifier( c ) ) {
							state->status = FALSE;
							if( !state->pvtError ) state->pvtError = VarTextCreate();
							vtprintf( state->pvtError, "fault while parsing object field name; \\u00%02X unexpected at %" _size_f "  %" _size_f ":%" _size_f, c, state->n, state->line, state->col );	// fault
							break;
						}
						if( !state->val.string ) state->val.string = output->pos;
	...
	while( input = DequeLink( &plqInputs ) )
                            ...
                            , size_t msglen )
{
	...
	retval = process_jsox_state( state, input, output );
