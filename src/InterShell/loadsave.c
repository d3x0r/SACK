...
static struct local_load_save_data {
	...
	PLINKSTACK current_page; // this is a stack of PPAGE_DATAs 
	genxStatus status;
...
static uintptr_t CPROC ResetCanvasConfig( uintptr_t psv, arg_list args )
{
	...
	PPAGE_DATA page = (PPAGE_DATA)PopLink( &l.current_page );
	...
	ShellSetCurrentPage( canvas, "first", PAGE_TRANSITION_NONE, 0 );
...
static uintptr_t CPROC ResetMainCanvasConfig( uintptr_t psv, arg_list args )
{
	...
	PCanvasData canvas = (PCanvasData)PeekLink( &l.current_canvas );
	PPAGE_DATA page = (PPAGE_DATA)PeekLink( &l.current_page );
	if( g.flags.multi_edit )
	{
		RestorePage( canvas, page, TRUE );
	}
...
#endif
				button = CreateSomeControl( page->frame, (int)col, (int)row, (int)width, (int)height, control_type_name );
				if( button )
				{
...
static uintptr_t CPROC SetMenuBackgroundColor( uintptr_t psv, arg_list args )
{
	...
	SetLink( &page->background_colors, 0, color );
	return psv;
}
...
static uintptr_t CPROC SetMenuBackground( uintptr_t psv, arg_list args )
{
	...
	PCanvasData canvas = (PCanvasData)PeekLink( &l.current_canvas );
	PPAGE_DATA page = (PPAGE_DATA)PeekLink( &l.current_page );
	if( page->background ) Release( (POINTER)page->background );
	if( page->background_image ) UnmakeImageFile( page->background_image );
	page->background = StrDup( filename );
	SetLink( &page->backgrounds, 0, StrDup( filename ) );
	SetLink( &page->background_images, 0, NULL );
...
static uintptr_t CPROC SetMenuBackgroundColorTheme( uintptr_t psv, arg_list args )
{
	...
	PCanvasData canvas = (PCanvasData)PeekLink( &l.current_canvas );
	PPAGE_DATA page = (PPAGE_DATA)PeekLink( &l.current_page );
	SetLink( &page->background_colors, (INDEX)theme_id, color );
...
static uintptr_t CPROC SetMenuBackgroundTheme( uintptr_t psv, arg_list args )
{
	...
	PCanvasData canvas = (PCanvasData)PeekLink( &l.current_canvas );
	PPAGE_DATA page = (PPAGE_DATA)PeekLink( &l.current_page );
	SetLink( &page->backgrounds, (INDEX)theme_id, StrDup( filename ) );
	SetLink( &page->background_images, (INDEX)theme_id, NULL );
	AddTheme( canvas, (int)theme_id );
	return psv;
}
...
uintptr_t CPROC SetMenuRowCols( uintptr_t psv, arg_list args )
{
	...
	//lprintf( "Page %p gets rows/cols %d/%d", page, (uint32_t)rows, (uint32_t)cols );
	button_space = 0;
	...
	if( button_cols == 0 )
		button_cols = 40;
	page->grid.nPartsX = button_cols;

	if( button_rows == 0 )
		button_rows = 40;
	page->grid.nPartsY = button_rows;
...
PPAGE_DATA InterShell_GetCurrentLoadingPage( void )
{
	return (PPAGE_DATA)PeekLink( &l.current_page );
}


static uintptr_t CPROC SetPageLayoutWide( uintptr_t psv, arg_list args )
{
...
static uintptr_t CPROC SetPageLayoutTall( uintptr_t psv, arg_list args )
{
	PCanvasData canvas = InterShell_GetCurrentLoadingCanvas( );
...
static uintptr_t CPROC CreateTitledPage( uintptr_t psv, arg_list args )
{
	PARAM( args, TEXTCHAR *, title );
	PCanvasData canvas = (PCanvasData)PeekLink( &l.current_canvas );
	PPAGE_DATA page = CreateNamedPage( canvas, title );
	PopLink( &l.current_page );
	PushLink( &l.current_page, page );
...
void AddCommonCanvasConfig( PCONFIG_HANDLER pch )
{
	...
	AddConfigurationMethod( pch, "page layout wide", SetPageLayoutWide );
	AddConfigurationMethod( pch, "page layout tall", SetPageLayoutTall );
...
void LoadButtonConfig( PCanvasData canvas, TEXTSTR filename )
{
	...
	PushLink( &l.current_page, canvas->default_page );
	PushLink( &l.current_canvas, canvas );
		...
		if( g.flags.bSQLConfig )
		{
         ...
         odbc = GetOptionODBC( g.configuration_dsn, g.configuration_version );
						               ...
						               , "intershell/configuration", name_only, &buffer, &buflen ) )
			{
				...
				out = sack_fopen( GetFileGroup( "Resources", NULL )
									 , alt_filename, "wb"
#ifdef _UNICODE
									  ", ccs=UNICODE"
#endif
...
void DumpGeneric( FILE *file, PMENU_BUTTON button )
{
	...
	if( button->pTypeName )
	{
		...
		if( button->flags.bListbox )
		{
			...
			fprintf( file, "%smulti select?%s lazy?%s\n"
                 , InterShell_GetSaveIndent()
					 , multi?"yes":"no"
		...
		else
		{
			...
			{
				fprintf( file, "auto generic parameters\n" );
				DumpCommonButton( file, button );
			}
		}
		if( button->show_on )
		...
		if( button->no_show_on )
		{
			...
			{
				fprintf( file, "%sDisallow show on %s\n", InterShell_GetSaveIndent(), GetText(name) );
			}
		}
		PopLink( &l.current_button );
	}
...
void SaveAPage( FILE *file, PPAGE_DATA page )
{
   ...
   if( page->title )
		fprintf( file, "page titled %s\n", page->title );

	fprintf( file, "page layout %d by %d\n", page->grid.nPartsX, page->grid.nPartsY );
	fprintf( file, "%sbackground color %s\n"
			 , (CDATA)GetLink( &page->background_colors, 0 )?"":"#"
			 , FormatColor( (CDATA)GetLink( &page->background_colors, 0 ) )
			 );
	fprintf( file, "%sbackground image %s\n"
			 , (CTEXTSTR)GetLink( &page->backgrounds, 0 )?((CTEXTSTR)GetLink( &page->backgrounds, 0 ))[0]?"":"#":"#"
			 , (CTEXTSTR)GetLink( &page->backgrounds, 0 )?EscapeMenuString( (TEXTSTR)GetLink( &page->backgrounds, 0 ) ):"" );
	{
		INDEX idx;
		CTEXTSTR background;
		...
		LIST_FORALL( page->background_colors, idx, CDATA, background_color )
		{
			if( !idx )
				continue;
			fprintf( file, "background color(%d) %s\n"
					 , idx
		...
		LIST_FORALL( page->backgrounds, idx, CTEXTSTR, background )
		{
			...
			fprintf( file, "background image(%d) %s\n"
					 , idx
...
void SaveButtonConfig( PCanvasData canvas, TEXTCHAR *filename )
{
   ...
   // -- additional code for XML output...
	...
	file = sack_fopen( GetFileGroup( "Resources", NULL ), filename, "wt" );
	if( file )
	{
...
#define fn(n) ((n)?EscapeMenuString(n):"")
		{
			...
			LIST_FORALL( g.glare_sets, idx, PGLARE_SET, glare_set )
			{
				...
				if( glare_set->flags.bShadeBackground )
					fprintf( file, "%s button mono shade\n", setname );
				if( glare_set->flags.bMultiShadeBackground )
					fprintf( file, "%s button multi shade\n", setname );
				fprintf( file, "%s button glare=%s\n", setname, fn(glare_set->glare) );
				fprintf( file, "%s button up=%s\n", setname, fn(glare_set->up) );
				fprintf( file, "%s button down=%s\n", setname, fn(glare_set->down) );
				fprintf( file, "%s button mask=%s\n", setname, fn(glare_set->mask) );
				{
					INDEX idx2;
