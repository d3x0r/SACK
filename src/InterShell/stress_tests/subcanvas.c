...
OnSaveControl( MY_CONTROL_NAME )( FILE *file,uintptr_t psv )
{
	SaveCanvasConfiguration( file, (MY_CONTROL_TYPE)psv );
}

OnLoadControl( MY_CONTROL_NAME )( PCONFIG_HANDLER pch, uintptr_t psvUnused )
{
	BeginCanvasConfiguration( (MY_CONTROL_TYPE)psvUnused );
}

OnGetControl( MY_CONTROL_NAME )( uintptr_t psv )
{
   return (PSI_CONTROL) ( (MY_CONTROL_TYPE)psv );
}

OnCreateControl( MY_CONTROL_NAME )( PSI_CONTROL frame, int32_t x, int32_t y, uint32_t w, uint32_t h )
{
