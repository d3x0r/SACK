...
static void CPROC TaskEnded( uintptr_t psv, PTASK_INFO task )
{
	...
	if( !l.task )
	{
		...
		tmp_task = LaunchPeerProgram( "pskill", NULL, args, KilTaskOutput, KillTaskEnded, 0 );
		if( tmp_task )
		{
...
static void OnFinishInit( "Crossfade Media Mount" )( PCanvasData pc_canvas )
{
	if( !l.waiting )
		l.waiting = ThreadTo( WaitForApplication, 0 );
}
...
static void OnSaveControl( "Crossfade Media Mount" )( FILE* file, uintptr_t psv )
{
	...
	fprintf( file, "Crossfade Server Address=%s\n", control->send_to?control->send_to:"" );
	fprintf( file, "Crossfade Send From Address=%s\n", control->send_from?control->send_from:"" );
}
