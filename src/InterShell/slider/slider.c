...
{
	PSLIDER_INFO info = (PSLIDER_INFO)psv;
	fprintf( file, "Slider color=$%02X%02X%02X%02X\n"
			 , AlphaVal( info->color )
			 , RedVal( info->color )
			 , GreenVal( info->color )
			 , BlueVal( info->color )
			 );
	fprintf( file, "Slider back color=$%02X%02X%02X%02X\n"
			 , AlphaVal( info->backcolor )
			 , RedVal( info->backcolor )
			 , GreenVal( info->backcolor )
			 , BlueVal( info->backcolor )
			 );
	fprintf( file, "Slider background image=%s\n", info->image_name?info->image_name:"" );
	fprintf( file, "Slider is horizontal?%s\n", info->flags.bHorizontal?"Yes":"No" );
	fprintf( file, "Slider is draggable?%s\n", info->flags.bDragging?"Yes":"No" );
	fprintf( file, "Slider min value=%d\n", info->min);
	fprintf( file, "Slider max value=%d\n", info->max);
	fprintf( file, "Slider current value=%d\n", info->current);
