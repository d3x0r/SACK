...
static LOGICAL ScanDirectory( struct volume *vol, const char * filename, FPI *dirFPI, struct directory_entry *dirent, struct directory_entry *dirkey, int path_match );
...
static int MaskStrCmp( struct volume *vol, const char * filename, FPI name_offset, int path_match ) {
	...
	if( vol->key ) {
		...
		if( path_match ) {
			size_t l;
			int r = PathCaseCmpEx( filename, dirname + name_offset, l = strlen( filename ) );
			if( !r )
			...
			return PathCaseCmpEx( filename, dirname, strlen(filename) );
	}
}
...
static LOGICAL ValidateBAT( struct volume *vol ) {
	...
	if( !ScanDirectory( vol, NULL, NULL, NULL, NULL, 0 ) ) return FALSE;
	return TRUE;
}
...
static POINTER _GetExtraData( POINTER block )
{
#ifdef WIN32
...
LOGICAL ScanDirectory( struct volume *vol, const char * filename, FPI *dirFPI, struct directory_entry *dirent, struct directory_entry *dirkey, int path_match ) {
	size_t n;
	BLOCKINDEX this_dir_block = 0;
	BLOCKINDEX next_dir_block;
	...
	while( 1 ) {
		...
		while( name < ( (unsigned char*)names + BLOCK_SIZE ) ) {
			...
			if( !c ) {
				...
				if( MaskStrCmp( vol, filename, (uintptr_t)name - (uintptr_t)names, 0 ) == 0 ) {
...
static struct directory_entry * _GetNewDirectory( struct volume *vol, const char * filename, FPI *entFPI, struct directory_entry *dirent, struct directory_entry *_entkey ) {
	size_t n;
	BLOCKINDEX this_dir_block = 0;
	struct directory_entry *next_entries;
...
struct sack_vfs_file * CPROC sack_vfs_openfile( struct volume *vol, const char * filename ) {
	...
	if( !ScanDirectory( vol, filename, &file->entry_fpi, &file->entry, &file->dirent_key, 0 ) ) {
		if( vol->read_only ) { LoG( "Fail open: readonly" ); vol->lock = 0; Deallocate( struct sack_vfs_file *, file ); return NULL; }
		else _GetNewDirectory( vol, filename, &file->entry_fpi, &file->entry, &file->dirent_key );
	}
	file->vol = vol;
	file->fpi = 0;
...
int CPROC sack_vfs_exists( struct volume *vol, const char * file ) {
	...
	result = ScanDirectory( vol, file, NULL, NULL, NULL, 0 );
	vol->lock = 0;
	return result;
}
...
size_t CPROC sack_vfs_write( struct sack_vfs_file *file, const char * data, size_t length ) {
	...
	while( length )
	{
		...
		if( length >= BLOCK_SIZE ) {
			MaskBlock( file->vol, file->vol->usekey[cache], block, 0, 0, data, BLOCK_SIZE );
...
static void sack_vfs_unlink_file_entry( struct volume *vol, FPI entFPI, struct directory_entry *entry, struct directory_entry *entkey, BLOCKINDEX first_block, LOGICAL deleted ) {
	...
	if( !file_found ) {
		...
		do {
			...
			//BLOCKINDEX b = BLOCK_SIZE + (block >> BLOCK_SHIFT) * (BLOCKS_PER_SECTOR*BLOCK_SIZE) + (block & (BLOCKS_PER_BAT - 1)) * BLOCK_SIZE;
			uint8_t* blockData = (uint8_t*)vfs_BSEEK( vol, block, &fileCache );
...
static void _shrinkBAT( struct sack_vfs_file *file ) {
...
size_t CPROC sack_vfs_truncate( struct sack_vfs_file *file ) { file->entry.filesize = file->fpi ^ file->dirent_key.filesize; _shrinkBAT( file ); return file->fpi; }
...
int CPROC sack_vfs_close( struct sack_vfs_file *file ) {
	...
	if( file->delete_on_close ) sack_vfs_unlink_file_entry( file->vol, file->entry_fpi, &file->entry, &file->dirent_key, file->first_block, TRUE );
	file->vol->lock = 0;
...
int CPROC sack_vfs_unlink_file( struct volume *vol, const char * filename ) {
	...
	if( ScanDirectory( vol, filename, &entFPI, &entry, &entkey, 0 ) ) {
		sack_vfs_unlink_file_entry( vol, entFPI, &entry, &entkey, entry.first_block ^ entkey.first_block, FALSE );
		result = 1;
	}
	vol->lock = 0;
	return result;
...
static int _iterate_find( struct find_info *info ) {
	struct directory_entry *next_entries;
	size_t n;
	do {
		enum block_cache_entries cache = BLOCK_CACHE_DIRECTORY;
...
int CPROC sack_vfs_find_first( struct find_info *info ) {
	...
	info->thisent = 0;
	return _iterate_find( info );
}

int CPROC sack_vfs_find_close( struct find_info *info ) { Deallocate( struct find_info*, info ); return 0; }
int CPROC sack_vfs_find_next( struct find_info *info ) { return _iterate_find( info ); }
...
LOGICAL CPROC sack_vfs_is_directory( uintptr_t psvInstance, const char *path ) {
	if( path[0] == '.' && path[1] == 0 ) return TRUE;
	{
		...
		if( ScanDirectory( vol, path, NULL, NULL, NULL, 1 ) ) {
			return TRUE;
		}
	}
	return FALSE;
...
LOGICAL CPROC sack_vfs_rename( uintptr_t psvInstance, const char *original, const char *newname ) {
	...
	if( vol ) {
		...
		FPI entFPI;
		while( LockedExchange( &vol->lock, 1 ) ) Relinquish();
		if( ( ScanDirectory( vol, original, &entFPI, &entry, &entkey, 0 ) ) ) {
			struct directory_entry new_entkey;
			struct directory_entry new_entry;
			if( (ScanDirectory( vol, newname, &entFPI, &new_entry, &new_entkey, 0 )) ) {
				vol->lock = 0;
