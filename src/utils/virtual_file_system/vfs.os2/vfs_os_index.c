...
struct memoryIndexEntry* loadIndexEntry( struct memoryStorageIndex* index, struct memoryIndexEntry* parent, uint64_t timeEntry ) {
	...
	if( !TESTFLAG( index->diskDataLoadedSectors, pos >> BLOCK_INDEX_SHIFT ) ) {
		...
		SETFLAG( index->diskDataLoadedSectors, pos >> BLOCK_INDEX_SHIFT );
	}

	// load next sector too, if it spans a boundary

	if( !TESTFLAG( index->diskDataLoadedSectors, (pos + ofs) >> BLOCK_INDEX_SHIFT ) ) {
		...
		SETFLAG( index->diskDataLoadedSectors, (pos + ofs) >> BLOCK_INDEX_SHIFT );
	}
