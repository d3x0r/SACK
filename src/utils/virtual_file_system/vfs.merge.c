	...
	if( vol->key ) {
		...
		while(  ( c = (dirname[name_offset] ^ vol->usekey[BLOCK_CACHE_NAMES][name_offset&BLOCK_MASK] ) )
			...
			if( path_match && !filename[0] ) {
				c = (dirname[name_offset&BLOCK_MASK] ^ vol->usekey[BLOCK_CACHE_NAMES][name_offset&BLOCK_MASK] );
				if( c == '/' || c == '\\' ) return 0;
			}
		}
		...
		if( path_match ) {
			size_t l;
			int r = PathCaseCmpEx( filename, dirname + (name_offset*BLOCK_MASK), l = strlen( filename ) );
			if( !r )
				if( (dirname + (name_offset&BLOCK_MASK))[l] == '/' || (dirname + (name_offset&BLOCK_MASK))[l] == '\\' )
					return 0;
				else
					return 1;
			return r;
...
static void MaskStrCpy( char *output, size_t outlen, struct volume *vol, FPI name_offset ) {
	if( vol->key ) {
		...
		//LoG( "doesn't volume always have a key?" );
		StrCpyEx( output, (const char *)(vol->usekey_buffer[BLOCK_CACHE_NAMES] + (name_offset & BLOCK_MASK )), outlen );
	}
}
#endif
...
static LOGICAL ValidateBAT( struct volume *vol ) {
	...
	if( vol->key ) {
		for( n = first_slab; n < slab; n += BLOCKS_PER_SECTOR  ) {
			...
			BLOCKINDEX *BAT; 
			BLOCKINDEX *blockKey; 
			BAT = TSEEK( BLOCKINDEX*, vol, n * BLOCK_SIZE, cache );//(BLOCKINDEX*)(((uint8_t*)vol->disk) + n * BLOCK_SIZE);
		...
		for( n = first_slab; n < slab; n += BLOCKS_PER_SECTOR  ) {
			size_t m;
			BLOCKINDEX *BAT = TSEEK( BLOCKINDEX*, vol, n * BLOCK_SIZE, cache );//(BLOCKINDEX*)(((uint8_t*)vol->disk) + n * BLOCK_SIZE);
			for( m = 0; m < BLOCKS_PER_BAT; m++ ) {
...
uintptr_t vfs_SEEK( struct volume *vol, FPI offset, enum block_cache_entries *cache_index ) {
	...
	vol->usekey_buffer[cache_index[0]] = ( offset & ~BLOCK_MASK );
...
uintptr_t vfs_BSEEK( struct volume *vol, BLOCKINDEX block, enum block_cache_entries *cache_index ) {
	...
	return vol->usekey_buffer[cache_index[0]] = ((uintptr_t)vol->disk) + (uintptr_t)b;
}
...
void sack_vfs_unload_volume( struct volume * vol ) {
	...
	if( vol->file ) sack_fclose( vol->file );
#else
...
//struct directory_entry * ScanDirectory( struct volume *vol, const char * filename, struct directory_entry *dirkey, int path_match ) {
	...
	do {
		...
		for( n = 0; n < VFS_DIRECTORY_ENTRIES; n++ ) {
			...
			if( bi == EODMARK ) return filename?NULL:(2); // done.
...
#if FILE_BASED_VFS
					if( dirent ) dirent[0][0] = (next_entries + n)[0];
#else
					if( dirent ) dirent[0] = (next_entries + n);
#endif
...
struct sack_vfs_file * CPROC sack_vfs_openfile( struct volume *vol, const char * filename ) {
	...
	if( !ScanDirectory( vol, filename, NULL, &file->entry, &file->dirent_key, 0 ) ) {
...
LOGICAL CPROC sack_vfs_rename( uintptr_t psvInstance, const char *original, const char *newname ) {
	...
	if( vol ) {
		...
		struct directory_entry *entry = &_entry;
		while( LockedExchange( &vol->lock, 1 ) ) Relinquish();
		if( ( entry  = ScanDirectory( vol, original, &entkey, 0 ) ) ) {
			struct directory_entry new_entkey;
			struct directory_entry _new_entry;
			struct directory_entry *new_entry = &_new_entry;
			if( (new_entry = ScanDirectory( vol, newname, NULL, &new_entry, &new_entkey, 0 )) ) {
				vol->lock = 0;
