...
static void loadImages( void ){
	...
	l.half_size = circle[15].pos.x * X_HALF_UNITS + image_srcs[9].low[0]->width;
	...
	l.full_temp = MakeImageFile( l.half_size*2, l.half_size * 2 );
	{
		...
		for( p = 0; p <= 5; p++ ) {
			...
			drawImages( l.half_temp, p, r, 0, 0, 0 );
			ClearImage( l.full_temp );
			drawImages( l.full_temp, p, r, 1, 0, 0 );
			{
				uint8_t *buf;
				size_t size;
...
SaneWinMain( argc, argv ) {
	char buf[256];
	if( argc > 1 ) {
		snprintf( buf, 256, "%s/straight-rail/", argv[1] );
		l.straight_path = StrDup( buf );
		snprintf( buf, 256, "%s/curved-rail/", argv[1] );
