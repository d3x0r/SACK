...
void CPROC ReadComplete( PCLIENT pc, POINTER buffer, size_t size )
{
	...
	else
	{
     	...
     	if( !LastMessage )
     	{
			...
			{
				printf( "Unknown responce from relay: %8.8s", (char*)buffer );
			}
		}
		else
		{
         ...
         else if( (test = ((*(uint64_t*)"WINNERS:")+1)), (LastMessage == test) )
         {
			...
			{
				Log( "(2)" );
				BasicMessageBox( "Relay Message", DupCharToText( (char*)buffer ) );
				LastMessage = 0;
			}
...
PCLIENT ConnectToRelay( PCLIENT *pc )
{
   ...
   ppc_current = pc;
   {
	   FILE *file = sack_fopen( 0, "LastConnection.data", "wb" );
	   if( file )
	   {
