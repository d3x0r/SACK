...
static struct json_context_object *WebSockInitJson( enum proxy_message_id message )
{
	...
	case PMID_PutString:
		...
		json_add_object_member( cto_data, "x", ofs = ofs + sizeof(uintptr_t), JSON_Element_Integer_32, 0 );
		json_add_object_member( cto_data, "y", ofs = ofs + sizeof(int32_t), JSON_Element_Integer_32, 0 );
		json_add_object_member( cto_data, "orientation", ofs = ofs + sizeof(int), JSON_Element_Integer_32, 0 );
		json_add_object_member( cto_data, "justification", ofs = ofs + sizeof(int), JSON_Element_Integer_32, 0 );
		json_add_object_member( cto_data, "width", ofs = ofs + sizeof(int32_t), JSON_Element_Integer_32, 0 );
		json_add_object_member( cto_data, "font_id", ofs = ofs + sizeof(uint32_t), JSON_Element_PTRSZVAL, 0 );
		json_add_object_member_user_routine( cto_data, "color", ofs = ofs + sizeof(uintptr_t), JSON_Element_Unsigned_Integer_32, 0, FormatColor );
		json_add_object_member_user_routine( cto_data, "background", ofs = ofs + sizeof(uint32_t), JSON_Element_Unsigned_Integer_32, 0, FormatColor );
		json_add_object_member( cto_data, "string", ofs = ofs + sizeof( uint32_t ), JSON_Element_CharArray, 0 );
		break;
	case PMID_FontData:
		json_add_object_member( cto_data, "server_font_id", ofs = 0, JSON_Element_PTRSZVAL, 0 );
		json_add_object_member( cto_data, "baseline", ofs = ofs + sizeof( uintptr_t ), JSON_Element_Unsigned_Integer_32, 0 );
		json_add_object_member( cto_data, "height", ofs = ofs + sizeof( uint32_t ), JSON_Element_Unsigned_Integer_32, 0 );
		json_add_object_member( cto_data, "bias_x", ofs = ofs + sizeof( uint32_t ), JSON_Element_Integer_8, 0 );
		json_add_object_member( cto_data, "bias_y", ofs = ofs + sizeof( uint8_t ), JSON_Element_Integer_8, 0 );
		//json_add_object_member( cto_data, "characters", ofs = ofs + sizeof(uintptr_t), JSON_Element_Integer_32, 0 );
		{
			int ofs_inner;
			struct json_context_object *cto_data_member;
			cto_data_member = json_add_object_member_list( cto_data, "data", ofs = ofs + sizeof(uint8_t), (enum JSON_ObjectElementTypes)0, 0 );
			json_add_object_member( cto_data_member, "c", ofs_inner = 0, JSON_Element_Integer_32, 0 );
			json_add_object_member( cto_data_member, "x", ofs_inner = ofs_inner + sizeof(uint32_t), JSON_Element_Integer_32, 0 );
			json_add_object_member( cto_data_member, "y", ofs_inner = ofs_inner + sizeof(int32_t), JSON_Element_Integer_32, 0 );
			json_add_object_member( cto_data_member, "w", ofs_inner = ofs_inner + sizeof(int32_t), JSON_Element_Integer_32, 0 );
			json_add_object_member( cto_data_member, "h", ofs_inner = ofs_inner + sizeof(uint32_t), JSON_Element_Unsigned_Integer_32, 0 );
			json_add_object_member( cto_data_member, "a", ofs_inner = ofs_inner + sizeof(uint32_t), JSON_Element_Integer_32, 0 );
			cto_data_member = json_add_object_member_list( cto_data, "colors", ofs = ofs + sizeof(PLIST), (enum JSON_ObjectElementTypes)0, 0 );
			json_add_object_member( cto_data_member, "color", ofs_inner = 0, JSON_Element_Unsigned_Integer_32, 0 );
			json_add_object_member( cto_data, "image_id", ofs = ofs + sizeof(PLIST), JSON_Element_PTRSZVAL, 0 );
		}
		break;
...
static char *Decode64Binary( uint8_t* buf, LOGICAL bmp, size_t length, size_t *outsize )
{
	char * real_output;
...
static uint8_t* EncodeImage( Image image, LOGICAL bmp, size_t *outsize )
{
	if( !bmp )
	{
		if( image )
	...
	case PMID_OpenDisplayAboveUnderSizedAt:
		{
			...
			((struct opendisplay_data*)(msg+5))->server_display_id = (uintptr_t)FindLink( &l.renderers, render );
	...
	case PMID_ImageData:
		{
			...
			if( outlen > (16000-5) && !websock )
			{
				...
				while( outlen > (16000-5) )
				{
					...
					lprintf( "Send Image %p %d  %d", image, image->id, ((uint32_t*)msg)[0] );
	...
	case PMID_DrawBlock:
		{
			...
			if( websock )
			{
				...
				msg = NewArray( uint8_t, sendlen = ( 4 + 1 + sizeof( struct image_data_data ) + outlen ) );
				outmsg = (struct common_message*)(msg + 4);
	...
	case PMID_PutString:
		{
			...
			CDATA color = va_arg( args, uint32_t );
			CDATA background = va_arg( args, uint32_t );
			TEXTCHAR *string = va_arg( args, TEXTCHAR * );
			size_t nChars = va_arg( args, uint32_t );
			SFTFont font = va_arg( args, SFTFont );
			int orientation = va_arg( args, int );
			int justification = va_arg( args, int );
			uint32_t right = va_arg( args, uint32_t );
			//uint8_t *msg;
			//size_t sendlen;
			//struct common_message *outmsg;
		#ifdef _UNICODE
			...
			//char *output = pc;
		#endif
			msg = NewArray( uint8_t, sendlen = ( 4 + 1 + sizeof( struct put_string_data ) + ( nChars + 1 ) * sizeof( TEXTCHAR ) ) );
			((uint32_t*)msg)[0] = (uint32_t)(sendlen - 4);
			outmsg = (struct common_message*)(msg + 4);
...
static void SendCompressedBuffer( PCLIENT pc, PVPImage image )
{
	if( image->websock_buffer && image->websock_sendlen )
	{
...
#endif						
		{
			...
			//lprintf( "Encode buffer (%d):%s",  CStrLen( encoded_data ), image->websock_buffer );
...
static void CPROC VidlibProxy_BlatColor	  ( Image pifDest, int32_t x, int32_t y, uint32_t w, uint32_t h, CDATA color )
{
	if( ((PVPImage)pifDest)->render_id != INVALID_INDEX )
	{
		...
		}
		{
			...
			AppendJSON( image, json_msg, ((uint8_t*)outmsg)-4, sendlen );
			Release( json_msg );
		}
											  ...
											  , uint32_t method, ... )
{
	...
	if( image->render_id != INVALID_INDEX )
	{
		...
		if( ( yd < 0 ) || ( yd +(int32_t)(hd&0x7FFFFFFF) ) > image->h )
		{
			int y = yd;
			int h = 0;
			//lprintf( "height fix : %d %d %d %d", yd, hd, image->y, image->h );
			while( yd < image->y )
			{
				erry += (signed)dhs;
				while( erry >= 0 )
			...
			//lprintf( "height fix : %d %d %d %d", y, yd, hd, hs );
			hs = 0;
			...
			//lprintf( "height fix : %d %d %d", y, h, hs );
			hd = h;
		}
		if( !wd || !hd )
			return;
...
static void VidlibProxy_SendPutString( PVPImage pImage, int32_t x, int32_t y, int32_t h, CDATA color, CDATA background
												, CTEXTSTR pc, size_t nLen, SFTFont font, int orientation, int justification, uint32_t right )
{
	if( !font )
	{
		font = l.real_interface->_GetDefaultFont();
												...
												, CTEXTSTR pc, size_t nLen, SFTFont font, int justification, uint32_t right )
{
	...
	((PVPImage)pImage)->image->flags |= IF_FLAG_UPDATED;
	VidlibProxy_SendPutString( (PVPImage)pImage, x, y, height, color, background, pc, nLen, font, 0, justification, right );
}
												...
												, CTEXTSTR pc, size_t nLen, SFTFont font )
{
	...
	((PVPImage)pImage)->image->flags |= IF_FLAG_UPDATED;
	VidlibProxy_SendPutString( (PVPImage)pImage, x, y, height, color, background, pc, nLen, font, 0, 0, 0 );
}
...
static void CPROC VidlibProxy_PutStringVerticalFontEx		( Image pImage, int32_t x, int32_t y, int32_t height, CDATA color, CDATA background, CTEXTSTR pc, size_t nLen, SFTFont font )
{
	...
	((PVPImage)pImage)->image->flags |= IF_FLAG_UPDATED;
	VidlibProxy_SendPutString( (PVPImage)pImage, x, y, height, color, background, pc, nLen, font, 1, 0, 0 );
}
...
static void CPROC VidlibProxy_PutStringInvertFontEx		  ( Image pImage, int32_t x, int32_t y, int32_t height, CDATA color, CDATA background, CTEXTSTR pc, size_t nLen, SFTFont font )
{
	...
	((PVPImage)pImage)->image->flags |= IF_FLAG_UPDATED;
	VidlibProxy_SendPutString( (PVPImage)pImage, x, y, height, color, background, pc, nLen, font, 2, 0, 0 );
}
...
static void CPROC VidlibProxy_PutStringInvertVerticalFontEx( Image pImage, int32_t x, int32_t y, int32_t height, CDATA color, CDATA background, CTEXTSTR pc, size_t nLen, SFTFont font )
{
	...
	((PVPImage)pImage)->image->flags |= IF_FLAG_UPDATED;
	VidlibProxy_SendPutString( (PVPImage)pImage, x, y, height, color, background, pc, nLen, font, 3, 0, 0 );
}
	...
	}
	{
		...
		outmsg->data.transfer_sub_image.image_to_id = ((PVPImage)pImageTo)->id;
		{
			TEXTSTR json_msg = json_build_message( cto, outmsg );
			{
				...
				//AppendJSON( ((PVPImage)pImageTo), json_msg, ((uint8_t*)outmsg)-4, sendlen );
			}
			Release( json_msg );
		}
		LeaveCriticalSec( &l.message_formatter );
