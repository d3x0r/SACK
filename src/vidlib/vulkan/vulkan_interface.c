...

	{
		...
		for( i = 0; i < deviceCount; i++ ) {
				...
				, VK_VERSION_PATCH( deviceProperties.apiVersion ) );
			{
				...
				VkQueueFamilyProperties *familyProperties = NewArray( VkQueueFamilyProperties, (queueFamilyCount) );
...
LOGICAL swapChainInit( struct SwapChain *swapChain )
{
	...
	VkBool32 *supportsPresent = NewArray( VkBool32, queueCount );
	...
	if( formatCount == 0 )
		return FALSE;
	VkSurfaceFormatKHR *surfaceFormats = NewArray( VkSurfaceFormatKHR, formatCount );
	if( swapChain->fpGetPhysicalDeviceSurfaceFormatsKHR( swapChain->physicalDevice,
		swapChain->surface,
		&formatCount,
	...
	void setImageLayout( VkCommandBuffer commandBuffer,
		...
		enum vkImageLayout oldImageLayout,
		enum vkImageLayout newImageLayout )
	{
	...
	uint32_t *height )
{
	...
	if( presentModeCount == 0 )
		return FALSE;
	VkPresentModeKHR *presentModes = NewArray( VkPresentModeKHR, presentModeCount );
	if( swapChain->fpGetPhysicalDeviceSurfacePresentModesKHR( swapChain->physicalDevice,
		swapChain->surface,
		&presentModeCount,
