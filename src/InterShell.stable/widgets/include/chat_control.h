...
CHAT_CONTROL_PROC( void, Chat_SetMessageExpire )( struct chat_widget_message *msg, int delta_seconds );
CHAT_CONTROL_PROC( void, Chat_ChangeMessageContent )( struct chat_widget_message *msg, CTEXTSTR text );
CHAT_CONTROL_PROC( void, Chat_SetMessageSeen )( struct chat_widget_message *msg );
CHAT_CONTROL_PROC( void, Chat_SetFonts )(PSI_CONTROL pc, SFTFont messages, SFTFont dates, SFTFont sender);
...
CHAT_CONTROL_PROC( int, Chat_ClearOldMessages )( PSI_CONTROL pc, int delete_time );
...
CHAT_CONTROL_PROC( void, Chat_AddAltMessageInputHandler )( PSI_CONTROL pc, const char *caption, void ( CPROC *Handler )( uintptr_t psv, PTEXT input ), uintptr_t psv );
CHAT_CONTROL_PROC( void, Chat_SetAltSendButtonSize )( PSI_CONTROL pc, uint32_t width, uint32_t height );
CHAT_CONTROL_PROC( void, Chat_SetAltSendButtonOffset )( PSI_CONTROL pc, int32_t x, int32_t y );
CHAT_CONTROL_PROC( void, Chat_SetAltSendButtonFont )( PSI_CONTROL pc, SFTFont font );
