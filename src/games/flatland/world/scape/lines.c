						  ...
						  , _POINT o, _POINT n )
{
...
#ifdef OUTPUT_TO_VIRTUALITY
	{
		...
		n2[1] = 10;
		n2[2] = -4;
		pls->pfl = CreateNormaledFacetLine( world->object, o, n, NEG_INFINITY, POS_INFINITY, n1, n2 );
	}
#endif
#ifdef WORLDSCAPE_SERVICE
	MarkLineUpdated( client_id, iWorld, ils );
#endif
	return ils;
}
...
INDEX SrvrDuplicateLine( _32 client_id, INDEX iWorld, INDEX ilsDup )
{
	...
	pls->dTo = plsDup->dTo;
#ifdef OUTPUT_TO_VIRTUALITY
	pls->pfl = CreateNormaledFacetLine( world->object, pls->r.o, pls->r.n, pls->dFrom, pls->dTo, VectorConst_X, VectorConst_Y );
#endif
	return ils;
}
...
//	if( pls->pfl )
 	...
 	if( !r )
	{
		...
		sprintf( msg, "Intersect N<%g,%g,%g> O<%g,%g,%g> with N<%g,%g,%g> O<%g,%g,%g>", 
							...
							pLine2->r.o[2] );
		Log( msg );
		sprintf( msg, "Result %d Times: %g and %g", r, t1, t2 );
		Log( msg );
