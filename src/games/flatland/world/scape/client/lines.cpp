...
int IntersectLines( PFLATLAND_MYLINESEG pLine1, int bEnd1, PFLATLAND_MYLINESEG pLine2, int bEnd2 )
{
	...
	{
		TEXTCHAR msg[256];
		snprintf( msg, 256, WIDE("Intersect N<%g,%g,%g> O<%g,%g,%g> with N<%g,%g,%g> O<%g,%g,%g>"), 
							pLine1->r.n[0], 
							pLine1->r.n[1], 
							pLine1->r.n[2], 
							pLine1->r.o[0], 
...
static void BalanceLine( PLINESEG pls )
{
...
void BalanceALine( INDEX iWorld, INDEX iWall, INDEX iLine, PLINESEG newseg, LOGICAL bLockSlope )
{
	BalanceLine( newseg );
	SendLineChanged( iWorld, iWall, iLine, newseg, FALSE, bLockSlope );
}

void GetLineData( INDEX iWorld, INDEX iLine, PFLATLAND_MYLINESEG *ppls )
{
	if( iLine != INVALID_INDEX )
	{
		PWORLD world = GetUsedSetMember( WORLD, &g.worlds, iWorld );
		(*ppls) = GetUsedSetMember( FLATLAND_MYLINESEG, &world->lines, iLine );
	}
	else
		if( ppls )
			(*ppls) = NULL;
