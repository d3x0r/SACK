...
void CPROC AddPlayerButton( uintptr_t psv, PSI_CONTROL pc )
{
	...
	else
	{
		...
		snprintf( buffer, 256, "Configure Player #%d", (nPlayers)+1 );
	...
	snprintf( buffer, 256, "Players: %d", nPlayers );
...
//      						, Colors[n] );
	...
	pFrame = CreateFrame( "Config Player", 0, 0, 275, 125, BORDER_NORMAL, (PSI_CONTROL)0 );
	pPlayerFrame = pFrame;
	...
	MakeButton( pFrame, 108, 100, 82, 20, BTN_ADD, "Add Player", 0, AddPlayerButton,  (uintptr_t)&nPlayers);
	MakeButton( pFrame, 14, 100, 82, 20, BTN_PLAY, "Play Game", 0, PlayGameButton, (uintptr_t)&done );
...
void ConfigureBoard( int *animate, int *sphere )
{
	PSI_CONTROL pFrame;
	PSI_CONTROL pc;
	pFrame = CreateFrame( "Config Board", 0, 0, 275, 275, BORDER_NORMAL, (PSI_CONTROL)0 );
	MoveFrame( pFrame, 50, 50 );
	...
	MakeButton( pFrame, 14, 200, 82, 20, BTN_PLAY, "Done", 0, PlayGameButton, (uintptr_t)&done );
	SetCheckState( MakeCheckButton( pFrame, 131, 200, 80, 14, CHK_ANIMATE, "Animate", 0, NULL, 0 ), 1 );
	SetCheckState( MakeCheckButton( pFrame, 131, 220, 140, 14, CHK_SPHERICAL, "Spherical Wrap", 0, NULL, 0 ), 1 );
