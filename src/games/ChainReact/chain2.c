	...
	for( i = 1; i <= maxplayers; i++ )
	{
		...
		if( players[i].active )
		{
		   BlotScaledImageSizedEx( GetDisplayImage( hDisplay ), pAtom
			...
			sprintf( buf, " : %d (%d)      ", players[i].wins, players[i].count );
...
void ExplodeInto( int x, int y, int into_x, int into_y )
{
	...
	// if it would explode
	// or there's only 1 in the square
	// then set the player to me
   // EXTENDED RULESET 1
	if( ( Board[into_x][into_y].count < 2 ) || ( CheckSquareEx( into_x, into_y, FALSE ) ) )
	{
         ...
         /* copy this, becuase it may be updating internally the state... */
			...
			for( n = 0; n < cnt; n++ )
			{
               ...
               r2.width = PeiceSize;
					...
					xy[n].__y = xy[n].y;
					//DebugBreak();
               //lprintf( "OUtput is at %d,%d to %d,%d (%d,%d) Total(%d,%d)", r1.x, r1.y, r2.x, r2.y, r1.width, r1.height, r.width, r.height );
	...
	while( 1 )
	{
      ...
      // assume we're destableized
		...
		for( bx = 0; bx < BOARD_X; bx++ )
			...
			{
				// if animated
				// then update board does one pass of queue'd suares?
...
int CPROC Mouse( PTRSZVAL dwUser, S_32 x, S_32 y, _32 b )
{
	...
	if( players[playerturn].computer )
	{
      //lprintf( "%d Computer's turn...", playerturn );
		//turn 1; // don't allow user updates for computer players
	}
			 ...
			 Boardy < BOARD_Y )
		{
			...
			if( (Boardy == (BOARD_Y-1)) &&
				...
				if( (!Boardy) &&
					(posy - HALFY) < -(BoardSizeY/2) )
					posy = (BoardSizeY/2) - HALFY;
         //lprintf( "AddPeice is %d,%d,%d,%d", Boardx, Boardy, posx, posy );
   ...
   SetBlotMethod( BLOT_MMX );
	{
		...
		{
			sprintf( players[n].name, "Player %d", n );
			players[n].color = n;
		}
	}
	...
	while(1)
	{
		...
		if( animate )
			while( unstable )
            Relinquish();
		if( ( 1 || ( ( !animate || !unstable ) ) ) &&
			 players[playerturn].computer && 
			!winstate )
		{
         lprintf( "Update... (computer move)" );
			ComputeMove( playerturn );
         lprintf( "Update... (ok...computer move)" );
		}
		else if( winstate )
		{
