...
SQInteger function_PlaceVoxel3D(HSQUIRRELVM v)
{
  ...
  OldVoxelType = Loc.Sector->Data.Data[Loc.Offset];
...
SQInteger function_MoveVoxel3D(HSQUIRRELVM v)
{
  ...
  DestVoxelType = LocDest.Sector->Data.Data[LocDest.Offset];
  ...
  SourceVoxelType = LocSource.Sector->Data.Data[LocSource.Offset];
...
SQInteger function_PushVoxelTo(HSQUIRRELVM v)
{
  ...
  S->GameEnv->World->GetVoxelLocation(&Loc, NewLocation.x, NewLocation.y, NewLocation.z );
  if (1!=S->GameEnv->VoxelTypeManager.VoxelTable[ Loc.Sector->Data.Data[Loc.Offset] ]->Interface_PushBlock_Push(&Loc,InVal2,1)) { sq_pushbool(v,SQFalse); return(1); };
  S->Extension->VoxelQuantity[SlotNum]--; if (S->Extension->VoxelQuantity[SlotNum] == 0) S->Extension->VoxelType[SlotNum] = 0;
...
SQInteger function_PullVoxelFrom(HSQUIRRELVM v)
{
  ...
  S->GameEnv->World->GetVoxelLocation(&Loc, NewLocation.x, NewLocation.y, NewLocation.z );
  if (1!=S->GameEnv->VoxelTypeManager.VoxelTable[ Loc.Sector->Data.Data[Loc.Offset] ]->Interface_PushBlock_Pull(&Loc,&VoxelType,1)) { sq_pushbool(v,SQFalse); return(1); };
