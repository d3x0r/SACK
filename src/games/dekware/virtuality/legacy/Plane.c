...
//         denom = ( ne * nc ) - ( nb * nf );
         if( NearZero( denom ) )
         {
            ...
            OutputDebugString( (char*)byBuffer );
#endif
            return FALSE;
         }
         else
...
void DestroyLinePSet( PLINESEGPSET plps )
{
   ...
   {
      if( plps->pLines )
         Deallocate(void*, plps->pLines );
      Deallocate(void*, plps );
   }
}
...
void AddLineToPlane( PLINESEG pl, PFACET pf )
{
   ...
   plps = pf->pLineSet;
   Allocate( plps, Lines ); // make sure there will be enough space....
...
{
   PLINESEG pl;
   Allocate( pls, Lines ); // make sure there's room...
   pl = pls->pLines + pls->nUsedLines++;
   pl->bUsed = TRUE;
   pl->bDraw = TRUE;
   pl->dFrom = rFrom;
...
PLINESEG CreateLineBetweenFacets( PLINESEGSET pls, PFACET pp1, PFACET pp2 )
{
...
//   LINESEG l1, l2;
  ...
  {
     OutputDebugString( "NON-SYMMETRIC!\n" );
  }
      /*
      else
      {
         ...
         {
            OutputDebugString("Intersect FAILED!\n");
         }
      }
      */
//   }
...
PFACET AddPlaneToSet( PFACETSET pps, PCVECTOR origin, PCVECTOR norm, char d )
{
   PFACET pp;

   Allocate( pps, Planes );    // make sure there's enough room in the plane set...
...
void DeletePlane( PFACETSET po, PFACET pp )
{
   ...
   Deallocate(void*, pp->pLineSet ); // don't need these at all...
...
int IntersectPlanes( PLINESEGSET pls, PFACETSET pps, int bAll )
{
   ...
   for( i = 0; i < pps->nUsedPlanes; i++ )
      for( j = i + 1; j < pps->nUsedPlanes; j++ )
      {
         ...
         OutputDebugString( (char*)byBuffer );
#endif
         pl = CreateLineBetweenFacets( pls,
                          pps->pPlanes + i,
...
int PointWithin( PCVECTOR p, PLINESEGPSET plps )
{
   int l1, l2;
   PLINESEG pl1, pl2 ;
