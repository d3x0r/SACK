#define ROTATION_DELTA 0.0150F
#define ROTATION_ACCEL 0.001F
#define KEY_VELOCITY 0.10F
#define KEY_ACCEL 0.01F
...
#define Inertia( KEY_LESS, KEY_MORE,KEY_LESS2, KEY_MORE2, va ) \
	...
	(ROTATION);
	Inertia(KEY_UP, KEY_DOWN, KEY_F, KEY_R, vRight );
	Inertia(KEY_LEFT, KEY_RIGHT, KEY_A,  KEY_D, vUp );
	Inertia(KEY_PGUP, KEY_HOME,  0, 0, vForward );

	(SPEED);
	Inertia( KEY_PGDN, KEY_END, KEY_R, KEY_Q, vRight );
