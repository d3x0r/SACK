...
void LAYER::Draw( PIBOARD board, Image image, int32_t x, int32_t y )
{
	...
	else
	{
		...
		if( pLayerData->atext )
		{
			SFTFont *ppFont = NULL;// UseACanvasFont( NULL, "Network Board Font" );
			if( ppFont )
...
LAYER::LAYER( PODBC odbc, PLIST peices, INDEX iLoadLayer )
{
	...
	}
	{
		...
		if( SQLRecordQueryf( odbc, NULL, &results, NULL
					  , "select x,y,min_x,min_y,width,height,linked_from_id,linked_from_x,linked_from_y,linked_to_id,linked_to_x,linked_to_y,route,peice_info_id,peice_type,stacked_on_id from board_layer where board_layer_id=%lu"
					  , iLoadLayer
					  ) && results )
		{
			...
			if( iStackedOn != INVALID_INDEX )
			{
				...
				if( !stacked_on )
				{
               lprintf( "((((((( PROBABLY A FATAL CONDITION! )))))))))))))" );
				}
				else
				{
