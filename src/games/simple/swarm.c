...
#define MASTER_FLEE_ACCEL_MAX  0.50//.0 // 50cm/s/s ... in
#define SLAVE_PURSUE_ACCEL_MAX 0.450//.0 // slightly slower accelleratino than master can flee at

#define UNIVERSAL_FRICTION (0.01)  /* applies such that v(n)=v(n-1)* UNIVERSAL_FRICTION */
#define BODY_TERMINAL_VELOCITY (60*MPH)
	int skip;
      ...
      PrintVector( o );
		...
		}
		{
			...
			LIST_FORALL( slaves, idx, POBJECT, slave )
			{
				...
				{
					VECTOR v;
					SetSpeed( slave->Ti, scale( v, GetSpeed( slave->Ti, v ), 1.0-(UNIVERSAL_FRICTION) ) );
				}
				GetOriginV( slave->Ti, so );
				sub( v, o, so );
				{
				...
				SetAccel( slave->Ti, v );
				Move( slave->Ti );
				RotateRel( slave->Ti, (double)rand()/(double)(100*RAND_MAX),(double)rand()/(double)(100*RAND_MAX),(double)rand()/(double)(100*RAND_MAX));
			}
		}
...
int main( void )
{
   ...
   v[2] = 0.75;
	//SetTimeScale( 0.5 );
	pWorld = CreateScaledInstance( CubeNormals, CUBE_SIDES, 0.03f, v, (PVECTOR)VectorConst_Z, (PVECTOR)VectorConst_X, (PVECTOR)VectorConst_Y );
