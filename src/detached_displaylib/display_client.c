...
struct display_client_tracker
{
	PLIST tracked_displays;  // these displays have been sent to the client.

};

struct display_client_local_tag
{
	PLIST tracked_displays;  // these displays have been sent to the client.

} display_client_local;
#define l display_client_local
...
static int CPROC ConnectToServer( void )
{
   ...
   AddIdleProc( (IdleProc)ProcessDisplayMessages, 0 );
	if( !g.flags.connected )
	{
		...
		{
			TEXTCHAR myname[256];
         snprintf( myname, "%s.Render", GetProgramName() );
			g.MsgBase = RegisterServiceHandler( myname, DisplayEventProcessor );
...
RENDER_PROC( PRENDERER, OpenDisplaySizedAt )   ( _32 attributes, _32 width, _32 height, S_32 x, S_32 y )
{
	if( ConnectToServer() )
	{
      ...
      InitCriticalSec( &rendeer->cs );
      renderer->pWindowPos.x = x;
      renderer->pWindowPos.y = y;
      renderer->pWindowPos.cx = width;
		renderer->pWindowPos.cy = height;
