...
static int CPROC ImageMessageHandler( _32 SourceRouteID, _32 Msg
												, _32 *data, _32 length
...
static int ConnectToServer( void )
{
   ...
   if( !g.flags.connected )
   {
      ...
      {
			TEXTCHAR myname[256];
         snprintf( myname, WIDE("%s.Image"), GetProgramName() );
         g.MsgBase = RegisterService( myname, ImageEventHandler );
...
IMAGE_PROC  void IMAGE_API  SetStringBehavior( Image pImage, _32 behavior )
{
	...
	init_args( args );
   PushArgument( args, Image, &MyImage->RealImage );
   TransactServerMessage( MSG_SetStringBehavior, args.args, args.argsize
								, NULL, NULL, 0 );
   PopArguments( args );
}
