					...
					, int asc, int desc )
{
	...
	fprintf( output, "// Character: size x,y ( %d, %d ) baseline = %d \n"
	...
	if( ((outwidth)/8)*szy )
		fprintf( output, "static struct{ char s, w, o, j, a, d; unsigned char data[%d]; } %s =\n", 
						((outwidth)/8)*szy, charid );    1
	else
		fprintf( output, "static struct{ char s, w, o, j, a, d; } %s =\n"
						, charid );

	fprintf( output, "{ %2d, %2d, %2d, 0, %2d, %2d", width, szx, xorg, asc, desc);
				...
				, int bywidth, int width, int height )
{
   ...
   printf( "--------------- Character compuation --------------------\n" );
	...
	if( width - max < 5 )
	{
      fprintf( stderr, "Character is wider than max metric %d\n", width-max );
	}
	if( min < 5 )
	{
      fprintf( stderr, "Character begins before baseline %d %d\n", min, character );
	}
	out = out_in;
	data = data_in;
	mindata = width;
   ...
   logfont.lfCharSet = 0;
	...
	while( 1 )
	{
		if(loop)
		{
			...
			if( !ChooseFont( &cf ) )
			{
				printf( "okay all done, thanx for dumping...\n" );
				return 0;
			}
		}
		loop = 1;
...

		{
...

			{
				...
				if( logfont.lfItalic )
					attribs[idx++] = 'I';
				attribs[idx] = 0;
				fprintf( output, "struct { unsigned short height, chars;\n"
