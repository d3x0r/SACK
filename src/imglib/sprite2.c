...
#ifndef AL_INLINE
   #define AL_INLINE(type, name, args, code)       type CPROC name args code;
#endif
#ifndef _AL_SINCOS
...
#define SCANLINE_DRAWER_GENERIC(name, INIT, PUTPIXEL)              \
   static void draw_scanline_##name(Image bmp, Image spr,      \
...
#define SCANLINE_DRAWER(bits_pp, GETPIXEL)                            \
   ...
   static void draw_scanline_modex(
    Image bmp, Image spr, fixed l_bmp_x, int bmp_y_i, fixed r_bmp_x,
 ...
 */
void _parallelogram_map(Image bmp, Image spr, fixed xs[4], fixed ys[4],
			...
			int sub_pixel_accuracy)
{
   ...
   //if (bmp->clip) {
      clip_left = bmp->x << 16;
      clip_right = ((bmp->x+bmp->width) << 16) - 1;
   //}
   //else {
   ...
   //if (bmp->clip) {
      if (clip_bottom_i > (bmp->y + bmp->height))
	 clip_bottom_i = (bmp->y + bmp->height);
   //}
   //else {
 ...
 */
void _parallelogram_map_standard(Image bmp, Image sprite,
 ...
 *  scales, etc.
 */
void _pivot_scaled_sprite_flip(Image bmp, Image sprite,
			       fixed x, fixed y, fixed cx, fixed cy,
			       fixed angle, fixed scale, int v_flip)
{
   fixed xs[4], ys[4];

   _rotate_scale_flip_coordinates(sprite->width << 16, sprite->height << 16,
...
IMAGE_PROC(  void, rotate_scaled_sprite )(ImageFile *bmp, SPRITE *sprite, fixed angle, fixed scale)
{
...
//{
   ...
   _pivot_scaled_sprite_flip(bmp, sprite, (sprite->hotx<<16) + (sprite->image->width * scale) / 2,
			     			      (sprite->hoty<<16) + (sprite->image->height * scale) / 2,
			     			      sprite->image->width << 15, sprite->image->height << 15,
			     			      angle, scale, FALSE);
//})
}
...
{
	rotate_scaled_sprite(bmp, sprite, angle, itofix(1));
}
...
IMAGE_PROC(  PSPRITE, MakeSpriteImageFileEx )( char *fname DBG_PASS )
{
   PSPRITE ps = MakeSpriteEx( DBG_VOIDRELAY );
