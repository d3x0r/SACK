/*
 *  This was nabbed from FreeImage
 *    I only wanted just this one format to add though; to sometimes
 *    read PNG and output ICO.. (in memory)
 *
 */
// ==========================================================
...
*/
static DWORD 
CalculateImageSize(Image* icon_dib) {
	DWORD dwNumBytes = 0;

	unsigned colors		= 256;//FreeImage_GetColorsUsed(icon_dib);
	unsigned width		= icon_dib->width;
	unsigned height		= icon_dib->height;
	unsigned pitch		= icon_dib->pitch;
...
*/
static DWORD 
CalculateImageOffset( Image *vPages, int nIndex ) {
	DWORD	dwSize;
