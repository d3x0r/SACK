...
#if !defined( __CYGWIN ) && !defined( __LINUX__ )
...
fixed fatan2(fixed y, fixed x)
{
   ...
   r = fdiv(y,x);
#else
   r = (int)(( (((float)y)/65536.0f) / (((float)x)/65536.0f))*65536.0);
#endif
...
fixed fsqrt(fixed x)
{
   ...
   : "%ecx", WIDE("%edx")                    /* clobbers ecx and edx */
   );
#else
   result = (int)( sqrt(((float)x)/65535.0)*65535.0 );
#endif
