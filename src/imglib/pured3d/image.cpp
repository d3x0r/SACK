...
IDirect3DBaseTexture9 *ReloadD3DTexture( Image child_image, int option )
{
...

	{
		...
		{
			D3DLOCKED_RECT rect;
			g_d3d_device->CreateTexture( image->width, image->height, 1, D3DUSAGE_DYNAMIC, D3DFORMAT::D3DFMT_A8R8G8B8, D3DPOOL::D3DPOOL_DEFAULT , &image_data->d3tex, NULL );
...
void InitShader( void )
{
	...
	if( !l.shader.flags.init_ok )
	{
		...
		l.shader.flags.init_ok = 1;
		{
			...
			glGenProgramsARB(1, &l.shader.multi_shader);
			result = glGetError();
			if( result )
			{
					lprintf( WIDE( "shader resulted %d" ), result );
			}
			glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, l.shader.multi_shader);
			{
				int result;
				char *program_string = ""
					//"!!NVfp4.0\n"
...
#if 0
				glUseProgramObjectARB( l.shader.multi_shader );
...

				{
					...
					int i;
					int type;
					//glGetProgramivARB( l.shader.multi_shader,  GL_ACTIVE_UNIFORMS, &count );
					...
					//glGetObjectParameterivARB (l.shader.multi_shader, GL_OBJECT_ACTIVE_UNIFORMS, &count);
					count = 3;
					// for i in 0 to count:
					//for( i = 0; i < count; i++ )
						//glGetProgramStringARB( GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, 
//						glGetActiveUniform  (l.shader.multi_shader, i, 4096, &length, &size, &type, buffer);
				}
				...
				glUseProgram( l.shader.multi_shader );
				{
					...
					glGetUniformIndices(l.shader.multi_shader, 3, names, indeces);
					lprintf( WIDE( "ya..." ) );
					result = glGetError();
					if( result )
					{
					...
					//glGetUniform
					result = glGetUniformLocation(l.shader.multi_shader, "c[0]");
					result = glGetUniformLocation(l.shader.multi_shader, "c");

					//glUniform1ui( l.shader.multi_shader, 0, 
					lprintf( WIDE( "ya..." ) );
				}
#endif
			}
