...
static __inline__ int my_sigemptyset(unsigned long *set)
{
...
static __inline__ int my_sigaddset(unsigned long* set, int signum) {
  int bit = signum - 1; // Signal numbers start at 1, but bit positions start at 0.
  unsigned long* local_set = (set);
...
#define sigaddset(s,n) my_sigaddset(s,n)
#define mygetpagesize() sysconf(_SC_PAGESIZE)
#else
#define mygetpagesize() getpagesize()
#endif
