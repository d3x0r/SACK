...
*/
		...
		if (ssl->alpnLen) {
			extSize = 2;
			messageSize += 4 + 1 + ssl->alpnLen; /* 4 type/len + 1 len + data */
		}
#endif
...
*/	
	...
	if (ssl->alpnLen) {
		...
		extLen += 4 + 1 + ssl->alpnLen; /* 4 type/len + 1 len + data */
	}
#endif
	...
	if (extLen != 0) {
		...
		*c = extLen & 0xFF; c++;
		...
		if (ssl->alpnLen) {
			...
			*c = EXT_ALPN & 0xFF; c++;
			/* Can only ever be a reply of one proto so totLen of +2 is fine */
	...
	if (extLen > 0) {
		...
		*c = extLen & 0xFF; c++;
		...
		if (userExt) {
			...
			while (ext) {
#ifdef USE_ALPN
				if (ext->extType == EXT_ALPN && ssl->extCb == NULL) {
					psTraceInfo("WARNING: Providing ALPN extension without registering extension callback to receive server reply\n");
				}
#endif
